...
- `php7.2.2-fpm-yii2`, `php-fpm-yii2` ([Dockerfile](./Dockerfile))
...
`docker pull bscheshir/codeception:php7.2.2-fpm-yii2`
...
docker build --pull --no-cache -t bscheshir/codeception:php7.2.2-fpm-alpine-yii2 -t bscheshir/codeception:php-fpm-alpine-yii2 -- .
docker push bscheshir/codeception:php7.2.2-fpm-alpine-yii2
...
```
sed -i -e "s/^FROM.*/FROM bscheshir\/php:7.2.2-fpm-4yii2/" Dockerfile
```
...
```
  codecept:
    image: bscheshir/codeception:php7.2.2-fpm-yii2
    depends_on:
      - php
    environment:
...
```
  browser:
    image: selenium/standalone-chrome-debug:3.9.1
    volumes:
