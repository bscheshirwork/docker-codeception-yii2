...
- `php7.1.11-fpm-yii2`, `php-fpm-yii2` ([Dockerfile](./Dockerfile))
...
`docker pull bscheshir/codeception:php7.1.11-fpm-yii2`
...
docker build --no-cache -t bscheshir/codeception:php7.1.11-fpm-yii2 -t bscheshir/codeception:php-fpm-yii2 .
docker push bscheshir/codeception:php7.1.11-fpm-yii2
...
```
sed -i -e "s/^FROM.*/FROM bscheshir\/php:7.1.11-fpm-4yii2/" Dockerfile
```
...
```
  codecept:
    image: bscheshir/codeception:php7.1.11-fpm-yii2
    depends_on:
      - php
    environment:
...
```
  firefox:
    image: selenium/standalone-firefox-debug:3.6.0
    ports:
      - '4444'
      - '5900'
```
