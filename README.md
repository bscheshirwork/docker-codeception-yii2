...
- `7.1.7-fpm-yii2`, `fpm-yii2` ([Dockerfile](./Dockerfile))
...
`docker pull bscheshir/codeception:php7.1.7-fpm-yii2`
...
docker build --no-cache -t bscheshir/codeception:php7.1.7-fpm-yii2 -t bscheshir/codeception:php-fpm-yii2 .
docker push bscheshir/codeception:php7.1.7-fpm-yii2
...
```
sed -i -e "s/^FROM.*/FROM bscheshir\/php:7.1.7-fpm-4yii2/" Dockerfile
```
...
```
  codecept:
    image: bscheshir/codeception:7.1.7-fpm-4yii2
    depends_on:
      - php
    environment:
...
## autocomplit
For smart IDE autocomplit copy source from the running container to `.codecept` using `docker cp` tools
```
...
```
  firefox:
    image: selenium/standalone-firefox-debug:3.3.1
    ports:
      - '4444'
      - '5900'
```
...
services:
  ...
  codecept:
    image: bscheshir/codeception:php7.1.7-fpm-yii2
    depends_on:
      - nginx
      - chrome
    environment:
