...
- `php7.3.12-fpm-yii2`, `php-fpm-yii2` ([Dockerfile](./Dockerfile))
...
`docker pull bscheshir/codeception:php7.3.12-fpm-yii2`
...
docker build --pull --no-cache -t bscheshir/codeception:php7.3.12-fpm-alpine-yii2 -t bscheshir/codeception:php-fpm-alpine-yii2 -- .
docker push bscheshir/codeception:php7.3.12-fpm-alpine-yii2
...
```sh
sed -i -e "s/^FROM.*/FROM bscheshir\/php:7.3.12-fpm-4yii2/" Dockerfile
```
...
```yml
  codecept:
    image: bscheshir/codeception:php7.3.12-fpm-yii2
    depends_on:
      - php
    environment:
      TZ: Europe/Moscow
...
services:
  php:
    image: bscheshir/php:7.3.12-fpm-4yii2-xdebug
    restart: always
    volumes:
